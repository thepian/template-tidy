...
function Translations (params, translations) {
  ...
  this.existing = {};
  for(var lang in this.params.existing) {
      var files = this.params.existing[lang];
      files.forEach(function(file) {
          if (fs.statSync(file).isFile()) {
              var content = JSON.parse(fs.readFileSync(file));
              this.addExisting(lang,content);
          }
      },this);
  }
  this.updateAllExisting();
...
Translations.prototype.addExisting = function(lang, forLang) {
    this.existing[lang] = this.existing[lang] || {};
    for(var key in forLang) {
        this.existing[lang][key] = forLang[key];
    }
};

Translations.prototype.updateAllExisting = function() {
    var all = this.existing.all = {};
    for(var lang in this.existing) {
        if (lang !== 'all') {
            for(var key in this.existing[lang]) {
                all[key] = all[key] || [];
                all[key].push(lang);
            }
        }
...
Translations.prototype.forAllExisting = function(cb,that) {
    Object.keys(this.existing.all).forEach(function(key) {
        cb.call(that, key, this.existing.all[key]);
    }, this);
};

Translations.prototype.setFoundKeys = function(keys) {
    this.foundKeys = keys;
};

Translations.prototype.forAllFoundKeys = function(cb,that) {
    Object.keys(this.foundKeys).forEach(function(key) {
        cb.call(that, key, this.foundKeys[key]);
    }, this);
};

Translations.prototype.isFoundKey = function(key) {
    return this.foundKeys[key] != undefined;
};

Translations.prototype.isFoundPrefix = function(key) {
    var parts = key.split('.'),
        found = false;
    parts.pop();
    for(var i=1; i<parts.length; ++i) {
        var star = parts.slice(0,i).concat('*').join('.');
        if (this.isFoundKey(star)) found = true;
    }
    return found;
};
...
Translations.prototype.getStatsLines = function() {
    return Object.keys(this._stats).map(function(key) {
